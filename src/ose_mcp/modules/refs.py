...
def register_refs(mcp):
    ...
    def search_refs(query: str, limit: int = 5) -> dict[str, Any]:
        """Search local OSE SRD cache/index."""
        with connect() as con:
            rows = con.execute(
                """
                SELECT p.id, p.title, p.url,
                       snippet(refs_fts, 1, '[', ']', '…', 20) AS snippet
                FROM refs_fts
    ...
    def open_ref(ref_id: int) -> dict[str, Any]:
        """Open a reference by id (returns full cached text + URL)."""
        with connect() as con:
            row = con.execute("SELECT id, title, url, text FROM refs_pages WHERE id=?", (ref_id,)).fetchone()
            if not row:
